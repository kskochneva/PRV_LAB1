...
int main()
{
    ...
    for (int N : sizes) {
        ...
        std::cout << "Для маленьких матриц (100×100):Даже однопоточная версия может быть быстрее многопоточной с мьютексами \n";

        std::cout << "Для средних матриц (500×500):Boost показывает ускорение ~2x для 2-4 потоков \n";

        std::cout << "Для больших матриц (1000×1000):Boost  наилучшее масштабирование (ускорение до 3-4x)Mutex : конкуренции за блокировку при каждой записи \n";
        std::cout << "\n";
    }



    //1. Boost.Thread (без блокировок)
    //Самый эффективный для всех размеров матриц и количества потоков


    //2. std::mutex с блокировками

    //Значительно медленнее для всех сценариев (обычно в 5-10 раз медленнее Boost версии)
    // Ускорение с ростом числа потоков минимально или отсутствует
